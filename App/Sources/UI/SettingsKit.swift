...
/// A number on a range: its label, then at the trailing edge the slider and the value it stands at, unit
/// included. `format` is given the value as it is stored, so a value the store clamps reads true.
...
/// trailing edge. The owner chose it for the six lid-close sounds; a new choice is still his to decide.
