...
public enum ActivityConstants {
    ...
    /// A shell seen at its prompt with no child started since the job began must be seen so again this much
    /// later before its job is dropped. The shell owns its terminal between `preexec` and the command's fork,
    /// and again between two commands of one line: milliseconds, which one sighting could catch and two this
    /// far apart do not. The same 5 s a command must run before it counts (`jobArmAfterDefaultSeconds`).
