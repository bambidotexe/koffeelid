...
/// A check that could not reach GitHub is tried again at the first tick an hour or more later; nothing of this
/// is kept across launches, because every launch checks anyway.
public struct UpdateSchedule: Equatable, Sendable {
    ...
    /// How often the app asks `isDue`. Shorter than `retryDelay`, so a retry comes 60 to 90 minutes after the
    /// failure, at the first tick once the delay has passed.
