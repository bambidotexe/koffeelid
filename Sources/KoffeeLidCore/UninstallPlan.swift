...
/// PATH, the login items, the Claude Code and Codex hooks, Copilot's hooks file, OpenCode's plugin and the
/// zsh snippet stay, and they go on running a binary that is no longer there on every event of theirs.
/// Everything in that list is removable from the app itself except the two files below, which are root-owned.
