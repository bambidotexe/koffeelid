...
child (`proc_listchildpids`). A shell's children are not only its commands: Powerlevel10k starts a
`gitstatusd` (`~/.cache/gitstatus/gitstatusd-darwin-arm64`) in the shell's own process group when the shell
starts and keeps it for the shell's life, and an earlier `&` job stays a child too; only a child forked after
a command began belongs to that command. `ProcWalk.info` reads the groups, `ProcWalk.childStartTimes` each
child's fork time. A builtin that blocks (`wait`, `read`, a loop of builtins) keeps the group and forks
nothing: it looks like the prompt. `exec`
...
nothing). A subshell that re-reads it (`(source ~/.zshrc; make)`) is still interactive and its `$$` is the
parent's, so its load ends the parent's running job.
...
and writes them back when the clip ends (the moments are in `functional.md` § Lid closed, lid open); a device
without a settable volume plays at its current level. After a restore the volume is read back, and a device
holding another value than the one written is logged (`volume override: device … holds …% after the restore`).
