...
- [ ] Page 2, "Éléments d’ouverture" says to turn KoffeeLid on under « Activité des apps en arrière-plan », which is where the switch really is, and Settings › Système shows the same sentence as its warning while the login item is missing
- [ ] Page 2, "Éléments d’ouverture" → System Settings comes forward and **stays** there; the wizard does not jump over it. Turn KoffeeLid on under « Activité des apps en arrière-plan », leave System Settings open: within about 2 s the row reads "Accordée" on its own with the wizard still behind
...
- [ ] Settings › System shows the sleep lock (`Disponible` / `Manquant`), Login Items (`Activé` / `Désactivé`) and the three permissions (`Accordée` / `Refusée`), a button under a row only while it is wrong; with the effect off a denied Screen Recording is blue, with it on it is red and Settings › Lid Effect shows the same row with its button and an orange warning; coming back from System Settings turns a row green within 2 s without closing the window; hovering the sleep lock and the Login Items rows shows their tooltip
...
- [ ] Settings › Auto-Arm: each hook reads green `Enabled` with a "Remove from …" button under it once done; Remove puts the row back to `Disabled` with a "Set Up …" button (uninstalls the 15 hooks / strips the block from ~/.zshrc); the Settings reset also removes both. With the auto-arm switch on and neither hook set up, both rows are orange and a warning sits under the first card; with the switch off, or with one hook set up, a missing hook is blue
