...
- [ ] Auto-armed by a command run in Terminal, then by one in another terminal app (iTerm, VS Code's terminal…): the badge is that app's icon each time; with Claude Code working as well, the Claude badge sits in front and the terminal's behind it, up and to the right; the badges stay on the cup after the work ends, through the whole "Auto-armed, off in N min" hold-off, and go with `disarmed (activity ended)`
