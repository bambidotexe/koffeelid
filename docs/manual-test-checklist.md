...
- [ ] Settings › Santé, walked in French and in English: the toolbar's stethoscope sits between Système and Don; opening the window on it, picking it and "Vérifier à nouveau" each show `Vérification` with a spinner on the "KoffeeLid" row (half a second at least after the button), then the summary; every group is there (Autorisations, Rester éveillé en sécurité, Batterie et chaleur, Après un plantage, Geste et effet de l’écran, Pendant que vous travaillez, Compatibilité, App, Rapport) and every row reads as `docs/functional.md` § The Health page says: on this Mac with everything set up the summary is green `Tout fonctionne`, Mode follows the menu (arm from it, then "Vérifier à nouveau"), Reprise après plantage is green `Active`, the last Claude Code event reads a few seconds after a turn, the lid angle follows the lid, the crashes row is green `Aucun`, "Emplacement" is `Applications`; hovering Mode shows the command line's status line and hovering "Suivi de Claude Code" says 15 of 15 events. Turn KoffeeLid off under « Activité des apps en arrière-plan » in Login Items: after "Vérifier à nouveau" that row is red with the stop sign, the summary red `Ne fonctionne pas : 1 problème`, the watchdog's row gone and an orange warning naming the switch under the card; turn it back on. Remove the Terminal hook on Settings › Activation auto: its row here is orange `Désactivé` with a warning pointing at that page, and the summary orange `1 point à vérifier`. "Copier le rapport" puts the whole page as text on the clipboard, `KoffeeLid <version>, macOS <version>` first; the page shows no version and no update
