...
- [ ] Ctrl+C a Copilot turn while its `sleep 60` runs → no hook fires (nothing in the journal); within ~35 s `activity: quiet Copilot turn … — transcript says aborted, turn over`, then the 30 min hold-off; a double Esc does the same
...
- [ ] Quit the app during a Copilot turn, Ctrl+C the turn, relaunch → `activity: quiet Copilot turn … — transcript says aborted, turn over` before the first count, and the session never counts
- [ ] A fake Copilot prompt, `echo '{"sessionId":"00000000-fake"}' | COPILOT_HOME=/tmp/kl-nowhere /Applications/KoffeeLid.app/Contents/MacOS/KoffeeLidHook hook copilot userPromptSubmitted` (no session-state folder there, so the line is kept, with no transcript path), left quiet for 20 s → once `activity: no transcript for Copilot session 00000000; staleness ends it`, and the session stays working until 2 h
