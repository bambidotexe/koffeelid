...
## Arming paths
- [ ] Left-click the mug icon → header "KoffeeLid — <mode>", then Off / Armed / Armed + screen on, each followed by its cup in grey (empty / closed eyes / round eyes, matching the menu bar glyph), with a checkmark on the current one; clicking a mode switches; the tooltip names the mode
- [ ] Auto-armed (Claude Code working, "While Claude Code…" on): the icon shows the closed-eyes cup with the Claude app's icon at its bottom-right corner (in colour, in a light and in a dark menu bar), the tooltip says "Auto-armed" and the greyed "Auto-armed while…" line ends with that same icon, while the menu still checks Off; `koffeelid arm` or ⌃⌥⌘L switches the glyph to the plain armed cup at once
- [ ] Auto-armed by a command run in Terminal, then by one in another terminal app (iTerm, VS Code's terminal…): the badge is that app's icon each time, hung off the cup's bottom-right corner over the foot of the handle and never over an eye, the cup itself tinted exactly like the neighbouring icons and not moving a pixel when the badge comes or goes; clicking the badge opens the menu like clicking the cup; with Claude Code working as well, the Claude badge sits in front and the terminal's behind it, up and to the right; Ctrl-C the Claude turn while the command runs → within a minute only the terminal's badge is left; the badges of what last ran stay on the cup through the whole "Auto-armed, off in N min" hold-off, and go with `disarmed (activity ended)`
...
- [ ] Page 3 "Arm while you work": the three hooks (Claude Code, Codex, Terminal), one "Set up…" button each, nothing clipped at the page's height (details under Auto-arm on activity)
...
- [ ] Settings › Auto-Arm › "Work running now" reads `Aucun`, then `Claude Code : 1, Codex : 0, commandes : 0` while a session works
...
- [ ] About 5 s after that uninstall, on the same Mac: `/Applications/KoffeeLid.app` is in the Trash, `/etc/sudoers.d/koffeelid` and `/usr/local/bin/koffeelid` are gone, `pmset -g | grep SleepDisabled` is 0, `~/Library/Application Support/KoffeeLid` is gone, `defaults read dev.rubens.koffeelid` fails and `~/Library/Preferences/dev.rubens.koffeelid.plist` does not exist at all (not an empty one), `~/Library/Caches/dev.rubens.koffeelid` and `~/Library/HTTPStorages/dev.rubens.koffeelid` are gone, `~/.claude/settings.json` has no KoffeeLid hook and no `settings.json.backup-koffeelid` beside it, `~/.codex/hooks.json` has no KoffeeLid hook, `~/.codex/config.toml` has no `[hooks.state."…hooks.json:…"]` table of KoffeeLid's and neither has a `.backup-koffeelid` beside it, `.zshrc` has no KoffeeLid block, `launchctl list | grep koffeelid` is empty and System Settings › Login Items shows no KoffeeLid
...
- [ ] `koffeelid install-hooks` → `Installed 15 Claude Code hooks -> /Applications/KoffeeLid.app/Contents/MacOS/KoffeeLidHook hook`; `koffeelid status` ends with `activity: 0 sessions working (Claude Code 0, Codex 0), 0 commands`
- [ ] `koffeelid install-hooks codex` → `Installed 12 Codex hooks -> /Applications/KoffeeLid.app/Contents/MacOS/KoffeeLidHook hook codex, trusted in ~/.codex/config.toml`; `~/.codex/hooks.json` holds the 12 events, `~/.codex/config.toml` ends with 12 `[hooks.state."/Users/…/.codex/hooks.json:<event>:0:0"]` tables (each with a `trusted_hash`), and both have a `.backup-koffeelid` beside them; in Codex, `/hooks` lists the 12 KoffeeLid hooks as trusted, none modified or untrusted
- [ ] Start a Codex turn (`codex` in a terminal, any prompt) → arms within a second of the prompt (`activity: running (1 session working (Claude Code 0, Codex 1), 0 commands)`, `auto-armed (activity)`); the menu's greyed line reads "Auto-armed while Codex works"; the turn ends → `activity: idle`, `auto-disarm scheduled in 1800s`; Codex's own output shows `hook: UserPromptSubmit` and `hook: Stop` lines
- [ ] Esc during a Codex turn → `activity: idle` at once (the `Interrupt` hook), then the 30 min hold-off; quitting Codex → its session gone (`SessionEnd`), `koffeelid status` back to `0 sessions working`
- [ ] Ctrl-C a Codex turn while its `sleep 60` runs → `Interrupt`, then a `PostToolUse` within a minute → `koffeelid status` stays at `0 sessions working`
...
- [ ] A fake Codex prompt for a made-up session id (`docs/development.md`, the `hook codex` line) left quiet for 20 s → once `activity: no rollout for Codex session …; the daemon or staleness ends it`, and the session stays working until its `Stop` or 2 h
- [ ] Quit the app during a Codex TUI turn, `chmod a-w ~/Library/Application\ Support/KoffeeLid/activity.jsonl` so the turn's `Stop` and the TUI's `SessionEnd` are lost, let the turn end and quit the TUI, `chmod u+w` it back, wait 30 min (the managed daemon drops a quit thread 30 min after it goes idle; a `codex app-server daemon stop` and start instead kills the pid the session recorded, and the launch prune drops it without this line), relaunch the app → `activity: Codex daemon has not loaded thread …, turn over` before the first count, and the session never counts
...
- [ ] Ctrl+C a Copilot turn while its `sleep 60` runs, the tool already allowed (`copilot -p --allow-all-tools …`, or approve `sleep` for the session first: an approval fires no hook, so the session reads `working` until that tool's `postToolUse`, up to ~30 s) → no hook fires on the interrupt itself (nothing in the journal); within ~35 s `activity: quiet Copilot turn … — transcript says aborted, turn over`, then the 30 min hold-off; a double Esc does the same
- [ ] A Copilot turn whose model call gives up (the network cut until Copilot stops retrying) → no `agentStop`; within ~35 s of the failure `activity: quiet Copilot turn … — transcript says failed, turn over`
- [ ] A Copilot turn whose tool runs `sleep 120` stays counted throughout, with no `quiet Copilot turn` line; a turn that runs a subagent (`task`) stays counted until the parent's own `agentStop`, never ended by the subagent's
- [ ] Quit the app during a Copilot turn, with its tool already allowed as above, Ctrl+C the turn, relaunch → `activity: quiet Copilot turn … — transcript says aborted, turn over` before the first count, and the session never counts
- [ ] A fake Copilot prompt, `echo '{"sessionId":"00000000-fake"}' | COPILOT_HOME=/tmp/kl-nowhere /Applications/KoffeeLid.app/Contents/MacOS/KoffeeLidHook hook copilot userPromptSubmitted` (no session-state folder there, so the line is kept, with no transcript path), left quiet for 20 s → once `activity: no transcript for Copilot session 00000000; staleness ends it`, and the session stays working until 2 h; clean up with `echo '{"sessionId":"00000000-fake"}' | COPILOT_HOME=/tmp/kl-nowhere /Applications/KoffeeLid.app/Contents/MacOS/KoffeeLidHook hook copilot sessionEnd` so the fake session does not sit working for the full 2 h
- [ ] `koffeelid install-hooks copilot` → `Installed 7 Copilot hooks -> /Applications/KoffeeLid.app/Contents/MacOS/KoffeeLidHook hook copilot`; `~/.copilot/hooks/koffeelid.json` holds the 7 events (`sessionStart`, `userPromptSubmitted`, `postToolUse`, `postToolUseFailure`, `notification`, `agentStop`, `sessionEnd`), each with `exec`, `args` and `timeoutSec`; a `copilot` session started after this reads them with no trust step, no restart
- [ ] `koffeelid uninstall-hooks copilot` → `Removed the Copilot hooks file.`; `~/.copilot/hooks/koffeelid.json` is gone; `copilot` starts without a warning; running it again → `No Copilot hooks file found — nothing to remove.`
- [ ] `echo '{"foo":1}' > ~/.copilot/hooks/koffeelid.json` (a file KoffeeLid did not write), then `koffeelid install-hooks copilot` → refuses, says the file already exists and is not one KoffeeLid wrote, `~/.copilot/hooks/koffeelid.json` unchanged
- [ ] `koffeelid install-hooks opencode` → `Installed the OpenCode plugin -> /Applications/KoffeeLid.app/Contents/MacOS/KoffeeLidHook hook opencode`; `~/.config/opencode/plugins/koffeelid.js` exists; `opencode plugin list` shows it with id `dev.rubens.koffeelid.opencode` and no `failed to load plugin` in `~/.local/share/opencode/log/opencode.log`
- [ ] `koffeelid uninstall-hooks opencode` → `Removed the OpenCode plugin.`; the file is gone; a running server drops it within a second (`opencode plugin list` no longer shows it); running the uninstall again → `No OpenCode plugin found — nothing to remove.`
- [ ] `echo '// not ours' > ~/.config/opencode/plugins/koffeelid.js` (a file KoffeeLid did not write), then `koffeelid install-hooks opencode` → refuses, says the file already exists and is not one KoffeeLid wrote, the file unchanged
- [ ] With Copilot's hooks and OpenCode's plugin both set up, Settings › System › "Reset KoffeeLid…" (or Uninstall) → the log lists `copilot hooks removed` and `opencode plugin removed`, both files gone
...
- [ ] Settings › Health: "Codex hooks" reads green `Enabled` with the tooltip `12 of 12 hook events point at this copy of KoffeeLid`; "Last Codex event" reads "None yet" until a turn, then `Just now` with the event's name in the tooltip; in Codex's `/hooks`, switch one KoffeeLid hook off → within 2 s "Codex hooks" reads orange `Disabled` on Health and on Auto-Arm, with "Set up Codex on the Auto-Arm page." under the table; Set Up Codex puts it back to `Enabled` and the hook back on
- [ ] `koffeelid uninstall-hooks codex` → `Removed KoffeeLid hooks from Codex.`; the 12 tables are gone from `~/.codex/config.toml` (a trust of another tool's hook, if any, stays), `~/.codex/hooks.json` keeps only other tools' hooks, and Codex starts without a warning
...
- [ ] While auto-armed the menu bar shows the closed-eyes cup wearing Terminal's icon and "Off" checked; the greyed header reads "KoffeeLid — Off" with "Auto-armed while a command runs" on the line under it; `koffeelid status` says `mode: off · auto-armed (activity)`
- [ ] Manual Off never ends an auto-arm: arm from the menu while a command runs, then Off → log `manual off (menu); the auto-arm holds the session`, the cup gets its badge back, the Mac stays armed; the command ends → hold-off → `disarmed (activity ended)`
...
- [ ] Ctrl-C a turn, wait for `turn over`, quit and reopen the app within 10 s → `replayed …` followed by no `armed (activity`
...
- [ ] Auto-armed, choose Armed + screen on from the menu → the round-eyes cup, `koffeelid status` says `mode: caffeinate · auto-armed (activity)`; the command ends → stays armed (manual); Off → disarms only if the level has dropped, otherwise back to the badged cup
...
- [ ] Settings › System › "Show Onboarding Again" → page 3 "Arm while you work" lists Claude Code, Codex and Terminal (zsh), one "Set up…" button each; Claude Code's writes the 15 hooks and flips the Settings switch on; Codex's writes the 12 hooks and their trust and flips it on too; Terminal's appends the "# ---------- KoffeeLid ----------" block once (a second click says already present), nothing clipped in en or fr
...
- [ ] Settings › Auto-Arm: each hook reads green `Enabled` with a "Remove from …" button under it once done; Remove puts the row back to orange `Disabled` with a "Set Up …" button (uninstalls the 15 hooks / the 12 Codex hooks and their trust / strips the block from ~/.zshrc); the Settings reset also removes all three. A missing hook is orange whatever the auto-arm switch; with the switch on and no hook set up a warning sits under the first card as well
- [ ] Menu bar: no hook set up → no "Disarm once finished" item; any hook set up → the item is there, unchecked
